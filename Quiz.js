...
scoreInput.innerHTML = ScoreInputResult++;
const finishGameAnnoucment = document.querySelector('.FinishGameAnnoucment')
if (trueStateName.innerHTML === stateNames[4]) {
    year.innerHTML = stateYears[4];
    capitals.innerHTML = stateCapitals[4];
}
...
let score = document.querySelector('.ResultName');
...
submit.addEventListener('click', function(event){
    if(scoreInput >= '10'){
        scoreInput.style.left = '105px'
    } else if(scoreInput === '50'){
        finishGameAnnoucment.innerHTML = 'The game is done! Click to the finish the game button to see the final score!'
        finishGameAnnoucment.style.color = 'white'
        finish.style.backgroundColor = 'darkred'
    }
...
if (scoreInput.value < 50) {
  document.querySelector('.scoreInput').setAttribute('aria-disabled', 'true');
}
finish.addEventListener('click', function(event){
    ...
    result.innerHTML = `${points}/50`
    if(points <= 10){
        status.innerHTML = `You probably don’t know most of the US states. You can try again to make your result better`
        status.style.color = 'darkred'
        score.style.color = 'darkred'
        result.style.color = 'darkred'
    } else if (points <= 20){
        status.innerHTML = `You know something about the US states’ cultures, and it’s a progress. But we would still want you to pass it again.`
        status.style.color = 'red'
        score.style.color = 'red'
        result.style.color = 'red'
    } else if(points <= 30){
        status.innerHTML = `You know about half of us states, maybe a little bit less or more, but there is a way to get better in this deal. Keep going.`
        status.style.color = 'yellow'
        score.style.color = 'yellow'
        result.style.color = 'yellow'
    } else if(points <= 40){
        status.innerHTML = `You are in the majority. You can talk about at least 5 states detailed, and you looking for something interesting in some states.`
        status.style.color = 'greenyellow'
        score.style.color = 'greenyellow'
        result.style.color = 'greenyellow'
    } else if(points <= 50){
        status.innerHTML = `You almost or absolutely know about every US state. You can tell something about at least 10 states.`
        status.style.color = 'green'
        score.style.color = 'green'
        result.style.color = 'green'
    } else{
        status.innerHTML = `It's seems like there's an error.`
    }
